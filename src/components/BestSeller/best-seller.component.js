...
const BestSeller = () => {
  ...
  SwiperCore.use([Navigation, Pagination, Controller, Thumbs]);
    ...
    bestSeller.forEach(element => {
      slides.push(
        <SwiperSlide key={`slide-${element.title}`} tag="li" className="bestSellerGrid">
        <a href={element.dest_url}>
          <img
            src={element.img}
            style={{ position: "relative", top: "15%", listStyle:"none",left:"30%" }}
          ...
          <label className="bestPrice">₺ {element.price}</label></a>
  ...
  return (
    <div className="bestProducts">
      ...
      <span className="allBest">
        TÜM ÇOK SATANLAR <FontAwesomeIcon icon={faArrowRight} size="m" />
      </span>
      <Swiper
